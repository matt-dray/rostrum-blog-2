...
description: Who do you call if there's a ferret in the lift shaft?
date: 2018-04-27
...
![](https://media.giphy.com/media/1w7Csq1wWJ1PG/giphy.gif){fig-alt="A large cat struggles to squeeze through a catflap." width=500}
...
![](resources/mr-chimney.jpg){fig-alt="A pigeon that's stuck inside a chimney breast." width=500}
...
This post was first written in April 2018, but was re-rendered with the latest rescue data as of `r Sys.Date()`.
:::
...
First, read [Local Authority District](https://en.wikipedia.org/wiki/Districts_of_England) (LADs) data directly from the [Open Geography Portal](http://geoportal.statistics.gov.uk/)[^geoportal] and then use the `st_as_sf()` function to make the conversion from the sp class to the sf class.

:::{.callout-note collapse=false appearance='default' icon=true}
This post was first written in April 2018, but was re-rendered with the latest boundary data as of `r Sys.Date()`.
:::
...
geojson_path <- "https://services1.arcgis.com/ESMARspQHYMw9BZ9/arcgis/rest/services/Local_Authority_Districts_May_2023_UK_BUC/FeatureServer/0/query?outFields=*&where=1%3D1&f=geojson"
...
[^geoportal]: I got the API query string [from the Open Geography portal API explorer](https://geoportal.statistics.gov.uk/datasets/ons::local-authority-districts-may-2023-uk-buc-2/api).