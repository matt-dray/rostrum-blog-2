...
![](resources/toot-hyde.jpg){fig-alt="Aerial image of Hyde Park, London, with three emoji mammoths overlaid, each one tooting on an emoji trumpet, with the word 'toot' coming out the end of the trumpet." width="100%"}
...
![](resources/rtoot-permission.png){fig-alt="A prompt on the Mastodon website that asks the user to provide authorisation for the rtoot package to have permission for read and write access. There's a big blue button that says 'authorise' and a big red one that says 'deny'." width="75%"}
...
Where `RTOOT_DEFAULT_TOKEN` is that API token from earlier, which is required for accessing Mastodon. As mentioned, it's stored as [a GitHub repo secret](https://docs.github.com/en/rest/actions/secrets) and called into the GitHub Action environment thanks to the `${{ secrets.RTOOT_DEFAULT_TOKEN }}` call in [the YAML file](https://github.com/matt-dray/londonmapbot/blob/main/.github/workflows/post-image.yml).
...
![](resources/londonmapbot-mastodon.png){fig-alt="Example of a toot on Mastodon from the londonmapbot account. The image is an aerial image of London. The text provides the latitude and longitude of the centre of the image and also an OpenStreetMap link." width="100%"}
