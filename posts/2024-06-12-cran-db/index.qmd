...
You can get this information in an R terminal, sure, but it's ripe for displaying in a web app that can be refreshed for the latest news. This seemed a good excuse for me to learn a couple of (newish-to-me) Shiny things: (a) [{bslib}](https://rstudio.github.io/bslib/) as a framework and (b) a way to create an arbitrary number of UI elements, since the number of packages with deadlines will vary from day to day.

In short, I made the Shiny(live) app, which is embedded below (may take a moment to fully load).

<iframe width='380' height='500' src='https://matt-dray.github.io/cran-deadlines/' title='CRAN deadlines'></iframe>
...
Some other new-to-me bits-and-bobs I used were [an emoji as a favicon](https://twitter.com/LeaVerou/status/1241619866475474946)[^favicon], {bslib}'s very simple [dark-mode toggle](https://rstudio.github.io/bslib/reference/input_dark_mode.html), and [`htmltools::tagAppendChild()`](https://rstudio.github.io/htmltools/reference/tagAppendChild.html) to splice breaks between cards in the `tagList`.

In the end, this was a good excuse chance to polish some Shiny knowledge. Of course, simpler approaches to displaying this information are possible. I started with just a basic {reactable} abnd I see [Hadley has now done something similar to that](https://hadley.github.io/cran-deadlines/)[^hadley], which allows you to click rows to expand a package's check results. It's a better tool[^shame].
