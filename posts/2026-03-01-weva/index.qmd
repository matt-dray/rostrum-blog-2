...
The [{weva} R package](https://github.com/matt-dray/weva) contains a titchy, opinionated command-line interface (CLI) to present an opinionated micro-weather report.
...
I've been using Python mostly because it seems a better tool for the job than R[^rscript].

So I was pleased to see a recent [blog post](https://tidyverse.org/blog/2026/02/rapp-0-3-0/) about a new release of [{Rapp}](https://cran.r-project.org/package=Rapp), an R package to help create CLIs.
...
So, because I have no imagination: introducing the concept [{weva}](https://github.com/matt-dray/weva/) R package ('wevvah'[^cockney]).
...
* the free APIs [Open-Meteo](https://open-meteo.com/) and [postcodes.io](https://postcodes.io/)
* [{httr2}](https://cran.r-project.org/package=httr2), [{lubridate}](https://cran.r-project.org/package=lubridate) and [{PostcodesioR}](https://cran.r-project.org/package=PostcodesioR)

### R console

From the R console, you could install[^dude] it like:
...
The mini display tells you the current and future temperature and weather.
...
Hopefuly your day will be brightened by the styling with [ANSI escape codes](https://en.wikipedia.org/wiki/ANSI_escape_code#Colors), assuming your terminal supports them.
...
Use the `--extremes` (`-e`) flag to also print the low and high temperatures for today.
...
This was made slightly more awkward because the hourly data is returned for the whole of the current day (i.e. some of it is from the past).
...
And so 24 hours from now is 10:00 tomorrow: 1 March 2026.

## Inside the crystal ball

What's actually happening in the code?

{weva} is just a normal R package, with one difference: [a script](https://github.com/matt-dray/weva/blob/main/exec/weva.R) under `exec/` that contains R code that {Rapp} will reinterpret into a CLI.
...
{weva} is very limited and opinionated with little attention to proper request-handling and testing.
Your kilometreage may vary[^km].
...
In fact, I already used this tool yesterday before a trip outside.
I discovered the day was going to be overcast with some showers.
Who could have guessed?

Reminds me of [Fran](https://github.com/francisbarton)'s observation when faced with differing forecasts:
...
[^tomorrow]: We don't need to forecast beyond that, becuase we know the weather's going to be [really dicey and unpredictable](https://en.wikipedia.org/wiki/The_Day_After_Tomorrow).
[^km]: This is becoming a catch-phrase of this blog, along with 'this does what I need to do'.