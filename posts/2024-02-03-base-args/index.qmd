...
`library()` calls are a staple of R scripts. Let's say I'm attaching the {lme4} package becuase I want to use the famous `cake` dataset[^baath].
...
Blimey, all we wanted was `cake`. But actually, we can be more selective with `library()` using the `include.only` argument (note thath you can `exclude` as well).
...
[^baath]: Rasmus recently [did some sleuthing](https://www.sumsar.net/blog/source-of-the-cake-dataset/) to discover the source of this dataset! A great read.