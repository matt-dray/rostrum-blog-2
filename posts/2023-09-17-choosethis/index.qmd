---
title: Executing R code with clickable console links
date: 2023-09-17
slug: choosethis
categories:
  - cli
...
[The {cli} R package](https://cli.r-lib.org/index.html) puts hyperlinks in the console that can execute code to help solve errors. Of course, I used this functionality for japes.

## Error!

Have you noticed that tidyverse error messages are both helpful and pretty? This is due to [Gabor's {cli} package](https://cli.r-lib.org/index.html). Sometimes I make errors on purpose just to see these messages (that's what I tell my colleagues, anyway). 

Have you noticed that sometimes the error message will include a link that, when clicked, will execute some code to help explore the bug? That's curious isn't it?
...
You can read more about hyperlinks [in the docs](https://cli.r-lib.org/reference/links.html). There's some limitations, including how your terminal must be capable of supporting this type of hyperlink (RStudio is capable). Note also that links are 'experimental'.

You might be wondering if a bad actor could exploit this to execute arbitrary code. As per [the {cli} documentation](https://cli.r-lib.org/reference/links.html?q=run#security-considerations), there are several restrictions in place:
...
I've got two quick sketches that use {cli}'s `.run`-enabled links. I've put these in [the {choosethis} package](https://github.com/matt-dray/choosethis), which is on GitHub.
...
This works by setting up a function for each option. Click a link and it will run that function, which itself will present more clickable options with their own underlying function. And so on. In this demo, the chain starts when the user runs `choosethis::begin()`.
...
To perform this check, you can use `cli::ansi_has_hyperlink_support()`. I added the argument `show_links` to the `choosethis::begin()` function that defaults to this:
