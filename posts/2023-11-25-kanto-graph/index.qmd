...
I extracted data from the disassembled code of Pokémon Red to create an interactive graph of the locations and their connections. You can [jump to the graph](#graph) below.
...
Today I'm back with the same map, but we're going to be far more methodical and abstract: we're going to extract data from the underlying game files to build a graph that describes all the locations and connections between them.

This is made possible by [the 'pret' collaborative](https://github.com/pret), who have reverse-engineered, or 'disassembled', a number of Pokémon games. This is interesting for digi-archaeological reasons, but also for our frivolous purposes.

## Assemble

To get a copy of the game's disassembled code, go to [the pret/pokered GitHub repo](https://github.com/pret/pokered), click the 'Code' button, then 'Download as zip' and finally unzip the file on your computer. 

We're interested in two subfolders: `/data/maps/objects/` and `/data/maps/headers/`. These both contain one Assembly file per named location in the game, like `AgathasRoom.asm` and `Bikeshop.asm`. We'll be handling these as simple text files.
...
So let's crack open the data and use some base R for corralling.
...
Now we can combine all the node pairs that share edges via either a simple connection or a warp. In this example we're interested in combinations, rather than permutations, of node pairs. This means that we can treat A→B and B→A as equivalent and simplify down to just A→B.
...
Of course, you could spend all day tweaking things like the physics of the relationship between each node, or you could replace each node an images of its in-game sprite. I've chosen to keep things relatively simple.
