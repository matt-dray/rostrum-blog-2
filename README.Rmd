...
get_post_info <- function(post_path) {
  ...
  data.frame(
    title = post_title,
    publish_date = lubridate::ymd(post_date)
  )
...
posts$link <- gsub("posts", "https://www.rostrum.blog", post_paths)
...
all_dates <- data.frame(
  publish_date = lubridate::as_date(
    lubridate::ymd(min(posts$publish_date)):lubridate::today()
  )
)
...
Source files for [rostrum.blog](https://www.rostrum.blog/): fun and learning with the R language. 

Built with [Quarto](https://quarto.org/) and deployed with [Netlify](https://www.netlify.com/).

## Stats

`r emoji("1234")` There have been `r nrow(posts)` posts on [the blog](https://www.rostrum.blog/) since `r min(posts$publish_date)`. `r if (format(today(), "%m-%d") == format(min(posts$publish_date), "%m-%d")) emoji::emoji("balloon")`

`r emoji("date")` That's a post roughly every `r round((as.numeric(today() - min(posts$publish_date))) / nrow(posts))` days, or about `r round(((nrow(posts) / (as.numeric(today() - min(posts$publish_date)))) * 30), 1)` posts per month.

`r emoji("new")` The latest post was published `r today() - max(posts$publish_date)` day(s) ago: [`r posts$title[1]`](`r posts$link[1]`).
...
Updated on `r format(Sys.time(), "%Y-%m-%d at %H:%M")`.